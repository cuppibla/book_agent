...
def save_activity_type_callback(
    ...
    # When an orchestrator calls a sub-agent, it uses the 'transfer_to_agent' tool.
    # We need to look at the arguments to see WHICH agent it transferred to.
...
def save_activity_type_callback1(
    tool,
    args: Dict[str, Any],
    tool_context: ToolContext,
    tool_response: Dict[str, Any],
    ...
    # Determine the type based on which agent was used
    if agent_name == "museum_expert":
        activity_type = "CULTURAL"
    elif agent_name == "restaurant_expert":
        activity_type = "FOOD"
    ...
    print(f"\n🔔 [CALLBACK] The planner just used the '{agent_name}'.")
